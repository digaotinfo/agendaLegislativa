 ...
 * @var array
 */
	//public $helpers = array('Html', 'Tinymce');
 ...
 */
	public function label($fieldName = null, $text = null, $options = array()) {
		...
		$schemas_table = $this->_introspectModel($this->model(), 'fields', $fieldName);

		//<span data-tooltip class="has-tip" title="Tooltips are awesome, you should totally use them!">extended information</span>
		if (!empty($schemas_table['comment'])){
			$text = '<span data-tooltip class="has-tip" title="'. $schemas_table['comment'] .'">'. $text .'</span>';
		}



		//print_r($schemas_table);
		if (empty($schemas_table['null'])):
			if ($schemas_table['type'] != 'boolean'):
				$text = $text . ' <small class="alert">*</small>';
			endif;
		endif;
 ...
 */
	public function inputs($fields = null, $blacklist = null, $zInput=null) {
		...
		foreach ($fields as $name => $options) {
			...
			$out .= $this->input($name, $options, $zInput);
		}
 ...
 */
	public function input($fieldName, $options = array(), $zInput=array()) {
		...
		if(strPos($fieldName, '_file')>0)		{ $options['type'] = 'file';	}
		if(strPos($fieldName, '_hidden')>0)		{ $options['type'] = 'hidden';	}
		//if(subStr($fieldName,0,4) == 'tags')	{ $options['type'] = 'hidden';	}
		if(subStr($fieldName,0,12) == 'url_amigavel')	{ $options['type'] = 'hidden';	}

		//if(strPos($fieldName, '_th_hidden') > 0) {
		//	print_r($options);
		//
		//}

		//print_r($this->_introspectModel($modelKey, 'fields', $fieldName));
		$schemas_table = $this->_introspectModel($modelKey, 'fields', $fieldName);
		//print_r($schemas_table);
		if (!empty($schemas_table['comment'])) {
			//$options['between'] = '<span class="italic between">'. $schemas_table['comment'] .'</span><br>';
		}

		//echo 'null: ['. $schemas_table['null'] .']';
		if (empty($schemas_table['null'])){
			if (!empty($schemas_table)){

				if ($schemas_table['type'] != 'boolean'){
					$options['required'] = '';

					if (!isset($options['pattern'])) {
						if ($schemas_table['type'] == 'text'){
							$options['pattern'] = '[a-zA-Z]+';

						} elseif ($schemas_table['type'] == 'number') {
							$options['pattern'] = '[0-9]';

						} elseif ($schemas_table['type'] == 'tel') {
							$options['pattern'] = '[0-9]+';
					...
					if (!isset($options['after'])) {
						$options['after'] = '';
						// $options['after'] = '<small class="error">Campo obrigatorio.</small>';
					} else {
						$options['after'] = '';
						// $options['after'] = '<small class="error">'. $options['after'] .'</small>';
					}
				}
			}
		...
		switch ($type) {
			case 'hidden':
				...
				if(strPos($fieldName, '_th_hidden')>0 && $this->request->params['action']=='edit' && $input['value']!='') {
					$nomeDoCampo = explode('+', preg_replace(array('/\[/', '/\]/', '/data/'), array('', '+', ''), substr($input['name'],0,-1)));

					$modelKeu  = $nomeDoCampo[0];
					$nomeCampo = $nomeDoCampo[1];

					//echo $input['name'];
					//print_r($input);
					// '.$this->Form->postlink('[excluir]', array('action' => 'deleteImage', $[$modelKey]['id'])).'
					$input = '<div id="'.$nomeCampo.'"><small><a href="javascript:void(0);" class="deleteImagem"><img src="'.$this->webroot.$input['value'].'" style="margin-top:-50px;" alt="'.$nomeCampo.'" /><br />[excluir]</a></small><br /><br /></div>';

					$input .= '<script>$(".deleteImagem").click(function(){var imagem = $(this).children("img").attr("alt"); $("#imagensToDelete").val( $("#imagensToDelete").val() + imagem +","); $("#'.$nomeCampo.'").hide();});</script>';

					$input .= '<input type="hidden" name="data['.$modelKey.'][deleteimagens]" id="imagensToDelete" />';
				}

				$format = array('input');

				if(strPos($fieldName, '_th_hidden')>0 && $input['value']!='') {
					unset($divOptions);
				}
			break;
			case 'checkbox':
			...
			case 'radio':
				if (isset($out['between'])) {
					$options['between'] = '<span class="form_between">'. $out['between'] .'</span>';
					$out['between'] = null;
				}
			...
			case 'file':
				...
				for($x=0 ;$x<count($explodeName); $x++) {
					$fieldID .= ucFirst($explodeName[$x]);
				}

				$fieldNameJS		 = $modelKey.$fieldID;
				$fieldNameControl	 = $modelKey.$fieldID.'Control';
				$fieldNameControlDiv = $modelKey.$fieldID.'ControlDiv';


				eval(file_get_contents('config/config.zoio'));

				if(!empty($zConfig[$modelKey][$fieldName]['width'])){
					$fileWidth		= $zConfig[$modelKey][$fieldName]['width'];
					$fileHeight		= $zConfig[$modelKey][$fieldName]['height'];
					$fileExtensions	= implode(', ', $zConfig[$modelKey][$fieldName]['types']);

					$dimensoes_file = 'Dimensoes: '.$fileWidth.'x'.$fileHeight.'px';
					if ($fileWidth == 0 && $fileHeight == 0):
						$dimensoes_file = '';
					endif;

					$fileDimensions = '<div><small>'.$dimensoes_file.'<br />Formatdos permitidos: '.$fileExtensions.'</small></div>';
				}else{
					$fileDimensions = '';
				}

				// Função para copiar o valor do input>File para o input>Hidden
				$options['onchange'] = 'javascript:setFileName("'.$fieldNameJS.'", "'.$fieldNameControl.'", "'.$fieldNameControlDiv.'");';
				$input = $this->file($fieldName, $options) . $fileDimensions;

				// Função para verificar se o nome do arquivo é valido
				$input .= $this->hidden($fieldName.'_Control', $options).'<div id="'.$fieldNameControlDiv.'" style="color:red">';
			...
			break;
			case 'textarea':
				//$input = $this->textarea($fieldName, array('class'=>'ckeditor'));
				$input = $this->textarea($fieldName, $options);
			break;
			case 'url':
		...
		if(array_key_exists($modelKey, $zInput) && array_key_exists($fieldKey, $zInput[$modelKey])) {
			if(array_key_exists('group', $zInput[$modelKey][$fieldKey])) {
				$labels='';
				$contents='';

				if($zInput[$modelKey][$fieldKey]['group']==false) {
					return '';
				}

				foreach($zInput[$modelKey][$fieldKey]['group'] as $key => $val) {
					if($val['type'] == 'textarea') {
						//$options['class'] = 'ckeditor';
					}

					$labels   .= '<li><a href="#'.$fieldKey.$val['position'].'tab" data-toggle="tab">'.$val['label'].'</a></li>';
					$contents .= '<div class="tab-pane" id="'.$fieldKey.$val['position'].'tab">'.$this->{$val['type']}($key, $options).'</div>';
				}

				//myTab
				$output = '
					<div class="tabbable">
						<ul id="'.$zInput[$modelKey][$fieldKey]['gtitle'].'" class="nav nav-tabs">
							<li class="active"><a href="#'.$fieldKey.'1tab" data-toggle="tab">'.$zInput[$modelKey][$fieldKey]['label'].'</a></li>
							'.$labels.'
						</ul>
						<div class="tab-content">
							<div class="tab-pane active" id="'.$fieldKey.'1tab">'.$this->{$zInput[$modelKey][$fieldKey]['type']}($fieldKey, $options).'</div>
							'.$contents.'
						</div>
					</div>
				';

				if($zInput[$modelKey][$fieldKey]['gtitle']) {
					$output = '<fieldset><legend>'.$zInput[$modelKey][$fieldKey]['gtitle'].'</legend>'.$output.'</fieldset>';
				}
			}
		}
