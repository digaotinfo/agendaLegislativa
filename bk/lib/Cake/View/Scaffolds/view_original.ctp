...
<h2><?php echo __d('cake', 'Visualizar %s', $singularHumanName); ?></h2>
	<dl>
<?php
$i = 0;
...
<div class="actions">
	...
	<ul>
<?php
	echo "\t\t<li>" . $this->Html->link(__d('cake', 'Editar %s', $singularHumanName),   array('action' => 'edit', ${$singularVar}[$modelClass][$primaryKey])) . " </li>\n";
	echo "\t\t<li>" . $this->Form->postLink(__d('cake', 'Deletar %s', $singularHumanName), array('action' => 'delete', ${$singularVar}[$modelClass][$primaryKey]), null, 'Tem certeza que deseja excluir esse registro?') . " </li>\n";
	echo "\t\t<li>" . $this->Html->link(__d('cake', 'Listar %s', $pluralHumanName), array('action' => 'index')) . " </li>\n";
	echo "\t\t<li>" . $this->Html->link(__d('cake', 'Adicionar %s', $singularHumanName), array('action' => 'add')) . " </li>\n";
...
<?php endif; ?>
	<div class="actions">
		<ul>
			<li><?php echo $this->Html->link(__d('cake', 'Editar %s', Inflector::humanize(Inflector::underscore($_alias))), array('controller' => $_details['controller'], 'action' => 'edit', ${$singularVar}[$_alias][$_details['primaryKey']]))."</li>\n";?>
		</ul>
	</div>
</div>
<?php
...
<?php endif; ?>
	<div class="actions">
		<ul>
			<li><?php echo $this->Html->link(__d('cake', "Adicionar %s", Inflector::humanize(Inflector::underscore($_alias))), array('controller' => $_details['controller'], 'action' => 'add'));?> </li>
		</ul>
	</div>
</div>
<?php endforeach;?>
