...
class RelatoriosController extends AppController{
	...
	public $uses = array('Relatorio', 'PlType', 'PlSituacao', 'Pl', 'LogAtualizacaoPl', 'Foco', 'OqueE', 'Situacao', 'NossaPosicao', 'User', 'Tema', 'StatusType', 'AutorRelator', 'PlAutorRelator');
	var $scaffold = 'admin';
        ...
        if(!empty($result)):
            ...
            if($registros[$model]['type'] == 'completoPDF'):
	            ...
	            foreach($result as $registro){
	                ...
	                $timeOqueE = $this->formatDate($registro['OqueE']['modified']);
	                $timeSituacao = $this->formatDate($registro['Situacao']['modified']);
	                $timeNossaPosicao = $this->formatDate($registro['NossaPosicao']['modified']);
                    if( !empty($registro['Justificativa']['justificativa']) && ($registro['StatusType']['id'] == 3)):
    	                $timeJustificativa = $this->formatDate($registro['Justificativa']['modified']);
