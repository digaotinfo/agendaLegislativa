...
class TinymceHelper extends AppHelper {
    ...
    * @return string An HTML textarea element with TinyMCE
    */
    function inputTiny($fieldName, $options = array(), $tinyoptions = array(), $preset = null){
        // If a preset is defined
        if(!empty($preset)){
            $preset_options = $this->preset($preset);
			...
			$preset_options = $this->preset('maquina');
			$tinyoptions = $preset_options;
		}
        $options['type'] = 'textarea';
        return $this->Form->inputTiny($fieldName, $options) . $this->_build($fieldName, $tinyoptions);
    }
    ...
    private function preset($name){
        ...
        if($name == 'maquina'){
            return array(
                'theme' => 'advanced',
                'plugins' => 'safari,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template',
                //'theme_advanced_buttons1' => 'bold,italic,underline,undo,redo,link,unlink,image,copy,cut,paste,forecolor,styleselect,removeformat,cleanup,code',
                'theme_advanced_buttons1' => 'bold,italic,underline,|,justifyleft,justifycenter,justifyright,justifyfull,|,cut,copy,paste,pasteword,|,undo,redo,|,bullist,numlist,blockquote,|,hr,link,unlink,image,|,code,preview',
                'theme_advanced_buttons2' => '',
                'theme_advanced_buttons3' => '',
                'theme_advanced_toolbar_location' => 'top',
                'theme_advanced_toolbar_align' => 'left',
