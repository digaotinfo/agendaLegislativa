...
//		$src_img_temp = $this->webroot.'img/upload_icon.gif';
		$src_img_temp = '';
...
$this->start('script');
	...
	function excluirImagem<?=$coluna_banco?>(acao) {
	...
	$(function(){
		...
		new AjaxUpload(btnUpload<?=$coluna_banco?>, {
			...
			name: '<?=$coluna_banco?>',
			data: ['<?=$model?>', '<?=$coluna_banco?>', '<?=$acao?>', '<?=$diretorio?>', '<?=$size['w']?>|<?=$size['h']?>', '<?=$th['width']?>|<?=$th['height']?>', '<?=$ext_string?>'],
			onSubmit: function(file, ext){
				...
				/////**************************/////
				/////**************************/////
				excluirImagem<?=$coluna_banco?>('upload');
			...
			onComplete: function(file, response){
				...
				//$('#<?=$coluna_banco?>_nome_imagem').val(file);
                $('#<?=$coluna_banco?>').attr('src', '<?=$this->webroot?>'+(response));
...
$this->end();
    ...
    // $this->request->data[$model][$coluna_banco] = $this->request->data[$model][$coluna_banco.'_nome_imagem'];
    // print_r($nome_imagem_banco);
?>
	...
	<div class="large-2 medium-3 small-6 columns text-center">
		...
		<div class="row">
			<div class="small-12 columns">
				...
				<a href="javascript:void(0);"  onClick="if(confirm('Tem certeza que deseja apagar este arquivo?')){excluirImagem<?=$coluna_banco?>('delete');}" class="radius button secondary small bt-talkinghub " id="del" style="margin-top:10px; margin-left:0 !important;">
					excluir
				</a>
			</div>
		</div>
	...
	<div id="upload" class="large-10 medium-9 small-6 columns">
		...
		<div class="row">
			<div class="small-12 columns">
				<span class="radius button secondary" id="<?=$coluna_banco?>_input">Anexar arquivo:</span>
			</div>
		</div>
		<?php
		if (!empty($max_width)){
			...
			<div class="row">
				<div class="small-12 columns">
					<p class="italic between">
						Tamanho máximo de <?=$max_width?>px x <?=$max_height?>px
					</p>
				</div>
			</div>
			<?php
