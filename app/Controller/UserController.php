...
class UserController extends AppController {
	...
	public $uses = array('User', 'Role', 'Empresa', 'Loguser');
    ...
    public function admin_log() {
		...
		if( $this->request->is('post') || $this->request->is('put') ){
			...
			if( !empty($buscar) ):
				$conditions = array(
					"OR" => array(
							   'User.name Like' => '%'.$buscar.'%',
							   'User.username Like' => '%'.$buscar.'%'
						   )
				);
    ...
    public function admin_index() {
		...
		if( $this->request->is('post') || $this->request->is('put') ){
			...
			if( !empty($buscar) ):
				$conditions = array(
					"OR" => array(
							   $model.'.name Like' => '%'.$buscar.'%',
							   $model.'.username Like' => '%'.$buscar.'%'
						   )
				);
				$this->paginate['conditions'] = array($conditions);
			endif;
    ...
    public function admin_add() {
        ...
        if ($this->request->is('post') || $this->request->is('post')) {
        	...
        	if (!empty($verificacao)) {
				...
				if($this->request->data['User']['newpassword'] != $this->request->data['User']['password_verify']){
		...
		$this->set('empresasAll', $this->Empresa->find('list', array('fields' => array('Empresa.id', 'Empresa.nome'), 'conditions' => array('Empresa.ativo' => 1))));
	...
	public function admin_gerarExcel($result=null){
        ...
        if(!empty($result)):
			...
			foreach ($result as $registro){
				...
				// >>> content
				$objPHPExcel->setActiveSheetIndex(0)
						->setCellValue('A'.$i, date('d/m/Y à\s H:i',strtotime($registro[$model]['created'])))
						->setCellValue('B'.$i, $registro['User']['username'])
						->setCellValue('C'.$i, $registro['User']['Role']['title'])
						->setCellValue('D'.$i, $registro[$model]['url']);
				// <<< content
			...
			$objWriter->save($root."uploads/acesso/". $nameFile);
			$a_url = array("url" => "uploads/acesso/". $nameFile);
